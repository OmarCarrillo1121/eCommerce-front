...
const Carrito = () => {
    ...
    const renderizarTarjetaPago = () => {
        ...
        const totalCompra = carrito.reduce((total, producto) => total + producto.quantity * producto.price, 0);
        return (
        <div className={styles['tarjeta-pago']}>
            ...
            <button onClick={() => buyGames(carrito)} disabled={carrito.length === 0}>Comprar</button>
        </div>
        );
    };
    ...
    return (
        <div>
        <h1>Carrito</h1>
            <button><Link to='/'>Catalogo</Link></button>
